...
** Keybindings
#+begin_src elisp
(global-set-key [f7] (lambda () (interactive) (xref-clear-marker-stack)))
#+end_src
* Utilities
** Browse at remote
Generate upstream GitHub URLs for a given file,commit,selection
...
'(debugpy-remote-attach modes
                        (python-mode python-ts-mode)
                        command ""
                        command-cwd
                        (projectile-project-root)
                        host "localhost" :type "python" :request "attach" :justMyCode nil :showReturnValue t
                        prefix-local
                        (projectile-project-root)
                        prefix-remote
                        (projectile-project-root)
                        fn
                        my-debugpy-path-mappings
                        ))
(defun my-debugpy-path-mappings (config)
  (plist-put config :pathMappings (vector (list :localRoot (plist-get config 'prefix-local)
                                                :remoteRoot (plist-get config 'prefix-remote))))
)
#+end_src
...
;; Add Dutch gpg password prompt regexp
(add-to-list 'magit-process-password-prompt-regexps "^Voer wachtwoordzin in: $")
#+end_src
